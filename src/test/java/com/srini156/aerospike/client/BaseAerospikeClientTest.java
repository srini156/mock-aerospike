...
public abstract class BaseAerospikeClientTest {
	...
	protected Bin[] bins1 = new Bin[] { new Bin("first", "first-1"), new Bin("second", 123L), new Bin("third", "third-1") };
	protected Key key2 = new Key(namespace, set, "Key2");
	protected Bin[] bins2 = new Bin[] { new Bin("first", "first-2"), new Bin("second", 124L), new Bin("third", "third-2") };
	...
	public void shouldExistRecords() {
		boolean[] result = aerospikeClient.exists(null, new Key[] { key1, key2, nonExistingKey });
	...
	public void shouldGetHeaders() {
		Record[] records = aerospikeClient.getHeader(null, new Key[] { key1, key2, nonExistingKey });
	...
	public void shouldGetMultipleRecords() {
		Record[] records = aerospikeClient.get(null, new Key[] { key1, key2, nonExistingKey });
		assertEquals(records.length, 3);
		// Assert first record
	...
	public void shouldGetMultipleRecordsFilteredBins() {
		Record[] records = aerospikeClient.get(null, new Key[] { key1, key2, nonExistingKey }, "first", "second");
		assertEquals(records.length, 3);
		// Assert first record
		Record firstRecord = records[0];
